...
export default function Qi() {
  ...
  const handleExport = async (format: "txt" | "docx" | "pdf" | "html" = "txt") => {
    const appName = getTranslation(language, "appName")

    switch (format) {
      case "txt":
        exportToTxt(content, appName)
        break
      case "docx":
        await exportToDocx(content, appName)
        break
      case "pdf":
        exportToPdf(content, appName)
        break
      case "html":
        exportToHtml(content, appName)
        break
    }
  }
  ...
  const handlePrint = () => {
    printDocument(content, getTranslation(language, "appName"))
  }
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-64">
                  ...
                  <DropdownMenuItem onClick={() => handleExport("pdf")}>
                    <FileCheck className="w-4 h-4 mr-2" />
                    <div className="flex flex-col">
                      <span>{getTranslation(language, "exportPdf")}</span>
                      <span className="text-xs text-muted-foreground">{getTranslation(language, "exportPdfDesc")}</span>
                    </div>
                  </DropdownMenuItem>
                  ...
                  <DropdownMenuItem onClick={() => handleExport("html")}>
                    <FileText className="w-4 h-4 mr-2" />
                    <div className="flex flex-col">
                      <span>{getTranslation(language, "exportHtml")}</span>
                      <span className="text-xs text-muted-foreground">
                        {getTranslation(language, "exportHtmlDesc")}
                      </span>
                    </div>
                  </DropdownMenuItem>
