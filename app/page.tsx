...
const splitContentIntoPages = (content: string, wordsPerPage = 250): string[] => {
...
export default function OnePageBinder() {
  ...
  const pages = viewMode === "book" ? splitContentIntoPages(content) : [content]
  const totalPages = pages.length
  const canGoPrevious = currentPage > 0
  const canGoNext = currentPage < totalPages - 2 // -2 because we show 2 pages at once
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto p-4">
        <div className="relative">
          ...
          {showSuggestion && viewMode === "single" && (
            ...
            <div className={`mx-auto ${getPageSizeClass()}`}>
              <div className="bg-card rounded-lg shadow-sm min-h-[600px]">
                <Textarea
                  ...
                  className="w-full h-full resize-none border-0 bg-transparent text-base leading-relaxed focus-visible:ring-0 min-h-[600px] p-8"
                />
              </div>
            </div>
          ) : (
            ...
            <div className="mx-auto max-w-6xl">
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="text-sm text-muted-foreground">
                  {t("app.book.pages")} {currentPage + 1}-{Math.min(currentPage + 2, totalPages)} {t("app.book.of")}{" "}
                  {totalPages}
                </div>
              ...
              <div className="flex gap-4 justify-center">
                {/* Left Page */}
                <div className="w-1/2 bg-white dark:bg-gray-900 rounded-lg shadow-lg border min-h-[800px] p-8 relative">
                  <div className="text-xs text-center text-muted-foreground mb-4">
                    {t("app.book.page")} {currentPage + 1}
                  </div>
                  {currentPage < totalPages && (
                    <div
                      className="font-serif text-sm leading-relaxed text-justify h-full"
                      style={{
                  ...
                  <div className="absolute bottom-4 left-0 right-0 text-xs text-center text-muted-foreground">
                    {currentPage + 1}
                  </div>
                </div>
                ...
                <div className="w-1/2 bg-white dark:bg-gray-900 rounded-lg shadow-lg border min-h-[800px] p-8 relative">
                  <div className="text-xs text-center text-muted-foreground mb-4">
                    {currentPage + 1 < totalPages ? `${t("app.book.page")} ${currentPage + 2}` : ""}
                  </div>
              ...
              {/* Edit Mode for Book View */}
              <div className="mt-8">
                <Card>
                  <CardContent className="p-4">
                    <Label className="text-sm font-medium mb-2 block">{t("app.book.editContent")}</Label>
                    <Textarea
                      ref={textareaRef}
                      value={content}
                      onChange={handleContentChange}
